...
Java_com_stanley_virtualmonitor_JNIUtils_testDecode(JNIEnv *env, jclass jcls,
                                                    jstring input_jstr,
                                                    jobject surface)
{
...
#pragma region 解码部分
  ...
  // 2.打开输入视频文件
    int t1=avformat_open_input(&pFormatCtx, input_cstr, NULL, NULL);
  if ( t1!= 0) {
      LOGE("%d/n",t1);
    LOGE("%s", "打开输入视频文件失败");
    return -1;
  ...
  // 4.视频解码，需要找到视频对应的AVStream所在pFormatCtx->streams的索引位置
